...
| Prompt presets        | Presets          | Prompt Presets           | `Presets` is the industry-standard label; use `Prompt Preset` where generation-specific clarity is needed. |
...
| Automations           | Agents           | Helpers                  | `Agents` is a general industry term for automated helpers without over-theming.                            |
| Provider connections  | Connections      | Provider Connections     | `Connections` is clearer and already matches the app surface text.                                         |
| Media/assets          | Media            | Assets                   | `Media` is clearer and already matches the app surface text.                                               |
