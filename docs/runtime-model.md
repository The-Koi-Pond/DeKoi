...
old row must still have a recognized provider plus non-empty native label,
base URL, and model fields. Removed provider lanes such as `mock`, `local`,
malformed `remote-runtime`, or missing `kind` still reject on the native load
path. Wider alias and shape cleanup belongs in the one-way legacy import path.
