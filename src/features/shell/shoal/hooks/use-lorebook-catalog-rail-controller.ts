...
export interface LorebookCatalogRailNav {
  ...
  setView: (
    view: { kind: "lorebooks"; lorebookId: string } | { kind: "lorebooks"; mode: "new-lorebook" },
  ) => void;
  view: {
    kind: string;
    lorebookId?: string;
...
export function useLorebookCatalogRailController({ nav }: UseLorebookCatalogRailControllerInput) {
  const [query, setQuery] = useState("");
  const normalizedQuery = query.trim().toLowerCase();
  const activeLorebookId = nav.view.kind === "lorebooks" ? (nav.view.lorebookId ?? null) : null;
  const filteredLorebooks = useMemo(
    () => filterLorebookCatalogRecords(nav.lorebooks, normalizedQuery),
    [nav.lorebooks, normalizedQuery],
  );
  const entryCount = useMemo(() => countLorebookCatalogEntries(nav.lorebooks), [nav.lorebooks]);
