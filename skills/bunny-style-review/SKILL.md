...
description: "Local-only branch/PR review workflow with failure-path and nitpick lenses, unrelated to the GitHub Actions Bunny Review automation in `.github/bunny-review`. Use when the user says bunny, asks for bunny-style-review, Bunny style review, local Bunny review, PR-style review, a second reviewer pass, or asks an agent to inspect a branch, PR, or diff for bugs, regressions, edge cases, proof gaps, existing tests, security, migration, performance, maintainability, realistic failure modes, or small review comments."
---
...
This is a local review skill for an agent reading the current workspace. It is
not the GitHub Bunny Review process.

If the user says only `bunny`, treat that as a request for this local review
skill. Treat it as the GitHub Bunny Review process only when the request
explicitly mentions GitHub Actions, CI, workflow dispatch, commit status,
`.github/bunny-review`, or `.github/workflows/bunny-review*.yml`.

Do not trigger, modify, dispatch, debug, or post through `.github/bunny-review`,
the `Bunny Review` commit status, `.github/workflows/bunny-review*.yml`, or
`pnpm check:bunny-review` just because this skill is active. Touch those files
only when the review target includes them or the user explicitly asks about the
GitHub Bunny Review automation.
...
4. Build the review context stack before deciding findings:
   - Active repo/team rules: current session instructions, `.github/agents/dekoi-workflow.md`, relevant repo-local skills, `.coderabbit.yaml`, `CLAUDE.md`, `.cursorrules`, GitHub Copilot instructions, or path-scoped docs when present.
...
For DeKoi reviews, use current repo sources as truth:

- Keep current session instructions, `.github/agents/dekoi-workflow.md`, and
  repo docs in force.
...
- Mode boundaries: shared helper or generation change silently affecting Messenger or Roleplay.
