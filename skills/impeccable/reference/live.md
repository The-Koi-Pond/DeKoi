...
```
LOOP:
  node skills/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```bash
node skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node skills/impeccable/scripts/detect-csp.mjs
```
