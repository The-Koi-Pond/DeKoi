...
- The bundled Universal V2 starter package is derived from Chai-authored,
  team-owned prompt material by deleting abandoned or redundant metadata and
  applying the DeKoi-owned outbound parameter-contract transformation described
  below. Prompt prose and choice text are unchanged.
...
| Checked-in derived artifact | Git blob `cac42b728be2d115305170e69d76dee0ff2efc06`, 39,559 bytes, SHA-256 `9a99c1da79bbec2ba10ebcf2e6b257d201a00d1e4974b2672ebe5712267bad98`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Transformation              | Delete `data.choiceBlocks[2].visibilityRule`; wrap the ten outbound parameter values in DeKoi's `{ send, value }` contract, with the three previously consumed fields enabled and the others disabled; normalize `reasoningEffort` from `maximum` to the native `max` enum; change the DeKoi package envelope from version 1 to 2, add native `data.preset.schemaVersion: 2`, remove package-level `data.preset.isDefault` because the current default authority lives outside Prompt Preset records, add each choice block's native required `label` equal to its existing `question` as metadata, and remove redundant `data.preset.variableOrder` and unused `data.preset.folderId` metadata. The `choiceBlocks` array and `defaultChoices` remain the order and default authorities. Prompt prose and choice text are unchanged. |
...
2. Remove `data.choiceBlocks[2].visibilityRule` from the attested artifact;
   apply only the parameter-contract, enum, and DeKoi v2 package-metadata
   transformations described above, including the two redundant metadata
   deletions; and verify that the resulting JSON equals the checked-in artifact.
...
Xel separately approved retaining and distributing the 39,559-byte derived
artifact after verifying the abandoned or redundant metadata deletions, the
DeKoi-owned parameter-contract transformation, and the later DeKoi v2 package
metadata conversion recorded above, including adding each choice block's native
required `label` equal to its existing `question`. None of these transformations
adds or alters prompt prose or choice text; they change only package metadata and
outbound parameter representation exactly as recorded above. This approval does
not claim that Chai attested the derived blob. The integrity check prevents the
reviewed derived artifact or this provenance record from being silently reused
for different prompt content.
...
`975ec5eb2f4fa1043e5b9683db366068278c15ef556734eb240a61f9cf4591ab`.
The maintainer-approved derived artifact is exactly 39,559 bytes, Git blob
`cac42b728be2d115305170e69d76dee0ff2efc06`, with SHA-256
`9a99c1da79bbec2ba10ebcf2e6b257d201a00d1e4974b2672ebe5712267bad98`.
