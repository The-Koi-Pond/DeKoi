...
#[cfg(test)]
use json_file::temporary_json_path;
pub(crate) use json_file::write_export_json_file;
use json_file::{
    backup_exists, backup_json_path, pre_repair_exists, pre_repair_json_path,
    recovery_artifacts_exist, sync_parent_directory_best_effort, temporary_json_exists,
    write_collection_json_file, write_repair_json_file,
};
...
mod tests {
    ...
    fn missing_collection_file_with_rollback_artifact_blocks_reads_and_writes() {
        ...
        let path = directory.join("characters.json");
        let rollback_path = directory.join("characters.json.rollback-test.tmp");
        fs::write(&rollback_path, "rollback").expect("rollback fixture should be created");
